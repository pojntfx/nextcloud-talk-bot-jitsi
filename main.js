...
function parse_args() {
    ...
    const argv = yargs
	  ...
	  .usage('$0 <options>')
	  .example('$0 -f nctalkbot-jitsi.yml', ' -> The file location is searched relative from calling path')
	  .alias('h', 'help')
	  .env('NCTALKBOT_')
	  .help('help')
	...
	if ( (ext  !== '.yml') && (ext !== '.yaml') ) {
	    throw new Error(`configfile '${argv.configfile}' has unsupported extension '${ext}'`)
	}
    ...
    let objectName = util.inspect(configData, showHidden=false, depth=0, colorize=true);
	...
	throw new Error(`configfile describes object '${objectValue}'. We do require '${programName}'!`);
...
const main = async () => {
    ...
    console.debug("nctalkbot-jitsi: config structure ...\nconfig:", util.inspect(config, showHidden=false, depth=2, colorize=true));
...
return
    ...
    log.info(`starting WebRTC node subsystem (timeout: ${config.nctalkbot.sleepTime} seconds)`);
    await jitsi.open();

    const bot = new NextcloudTalkBot(config.nctalkbot.nctalkproxydAddr);

    log.info(`connecting to nctalkproxyd with address ${nctalkproxydAddr}`);
	...
	const password = crypto
	      .randomBytes(parseInt(config.nctalkbotRoomPasswordLength))
	      .toString("hex");
	...
	await jitsi.createRoom(config.nctalkbot.jitsiAddr, token,
			       config.nctalkbot.botName, password,
